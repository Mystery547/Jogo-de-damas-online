	...
	def desenha(self):
		...
		for l in range(len(matriz)):
			...
			for c in range(len(matriz[l])):
				if matriz[l][c] == '#':
					pygame.draw.rect(display, color.cor_tabuleiro, (x, y, 75, 75))
				else:
					pygame.draw.rect(display, color.branco, (x, y, 75, 75))
				x += 75
			y += 75
		...
		if self.cedula_selecionada:
			...
			if obrigatorios != {}:
				if (self.cedula_selecionada[0], self.cedula_selecionada[1]) not in obrigatorios:
					...
					pygame.draw.rect(display, color.vermelho_claro, (x_vermelho, y_vermelho, 75, 75))
				else:
					if movs[0] == []:
						x_vermelho = ALTURA / 8 * self.cedula_selecionada[1]
						y_vermelho = ALTURA / 8 * self.cedula_selecionada[0]
						...
						for i in range(len(movs[0])):
							...
							pygame.draw.rect(display, color.verde_claro, (x_possivel, y_possivel, 75, 75))
			else:
				if self.pulando:
					x_vermelho = ALTURA / 8 * self.cedula_selecionada[1]
					y_vermelho = ALTURA / 8 * self.cedula_selecionada[0]
		...
		for l in range(len(self.matriz_jogadores)):
			for c in range(len(self.matriz_jogadores[l])):
				...
				if elemento != '-':
					...
					if elemento.lower() == 'x':
						pygame.draw.circle(display, color.vermelho, (x, y), 20, 0)
						if elemento == 'X':
							pygame.draw.circle(display, color.preto, (x, y), 10, 0)
							pygame.draw.circle(display, color.azul, (x, y), 5, 0)
					else:
						pygame.draw.circle(display, color.branco, (x, y), 20, 0)
						if elemento == 'O':
							pygame.draw.circle(display, color.preto, (x, y), 10, 0)
							pygame.draw.circle(display, color.azul, (x, y), 5, 0)
		...
		if self.status != 'Game Over':
			...
			surface_texto, rect_texto = text_objects("Vermelho: " + str(12 - o), fonte, color.vermelho_claro)
			rect_texto.center = (650, 30)
			display.blit(surface_texto, rect_texto)

			surface_texto, rect_texto = text_objects("Branco: " + str(12 - x), fonte, color.branco)
			rect_texto.center = (650, ALTURA - 30)
			display.blit(surface_texto, rect_texto)

			if self.turno % 2 == 1:
				surface_texto, rect_texto = text_objects("Turno do branco", fonte, color.branco)
				rect_texto.center = (700, ALTURA / 2)
				display.blit(surface_texto, rect_texto)
			else:
				surface_texto, rect_texto = text_objects("Turno do vermelho", fonte, color.vermelho_claro)
				rect_texto.center = (700, ALTURA / 2)
				display.blit(surface_texto, rect_texto)
		else:
			surface_texto, rect_texto = text_objects("Game Over", fonte, color.azul)
